...
export function loadProfiles() {
  try {
    ...
    return { profiles: {}, active: null, mode: null };
  }
}
...
export function setMode(mode) {
  if (mode !== 'isolated' && mode !== 'unified') {
    throw new Error('Mode must be "isolated" or "unified"');
  }
  const data = loadProfiles();
  data.mode = mode;
  saveProfiles(data);
 ...
 * In unified mode, all profiles share one config dir.
 * In isolated mode, each profile gets its own.
 */
